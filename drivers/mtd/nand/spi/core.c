...
#if IS_ENABLED(CONFIG_MTD_SPI_NAND_ALLIANCEMEMORY)
	&alliancememory_spinand_manufacturer,
#endif
#if IS_ENABLED(CONFIG_MTD_SPI_NAND_ATO)
	&ato_spinand_manufacturer,
#endif
#if IS_ENABLED(CONFIG_MTD_SPI_NAND_ESMT)
	&esmt_c8_spinand_manufacturer,
#endif
#if IS_ENABLED(CONFIG_MTD_SPI_NAND_FMSH)
	&fmsh_spinand_manufacturer,
#endif
#if IS_ENABLED(CONFIG_MTD_SPI_NAND_FORESEE)
	&foresee_spinand_manufacturer,
#endif
#if IS_ENABLED(CONFIG_MTD_SPI_NAND_GIGADEVICE)
	&gigadevice_spinand_manufacturer,
#endif
#if IS_ENABLED(CONFIG_MTD_SPI_NAND_MACRONIX)
	&macronix_spinand_manufacturer,
#endif
#if IS_ENABLED(CONFIG_MTD_SPI_NAND_MICRON)
	&micron_spinand_manufacturer,
#endif
#if IS_ENABLED(CONFIG_MTD_SPI_NAND_PARAGON)
	&paragon_spinand_manufacturer,
#endif
#if IS_ENABLED(CONFIG_MTD_SPI_NAND_SKYHIGH)
	&skyhigh_spinand_manufacturer,
#endif
#if IS_ENABLED(CONFIG_MTD_SPI_NAND_TOSHIBA)
	&toshiba_spinand_manufacturer,
#endif
#if IS_ENABLED(CONFIG_MTD_SPI_NAND_WINBOND)
	&winbond_spinand_manufacturer,
#endif
#if IS_ENABLED(CONFIG_MTD_SPI_NAND_XTX)
...
static int spinand_ofdata_phy_pattern(struct udevice *dev)
{
	...
	while (ofnode_valid(sub_partition_node)) {
		...
		if (label && strstr(label, "phypattern")) {
			if (!ofnode_read_u32_array(sub_partition_node, "reg",
						   &start, 1))
				return start;
...
static int spinand_probe(struct udevice *dev)
{
	...
	ret = spinand_wait(spinand, SPINAND_ERASE_INITIAL_DELAY_US,
			   SPINAND_ERASE_POLL_DELAY_US, &status);
	if (ret < 0)
		goto err_spinand_cleanup;

	ret = spi_mem_do_calibration(spinand->slave, &read_page_op);
