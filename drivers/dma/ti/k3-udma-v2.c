...
static const char * const mmr_v2_names[] = {
	[MMR_V2_GCFG] = "gcfg",
	[MMR_V2_BCHANRT] = "bchanrt",
	[MMR_V2_CHANRT] = "chanrt",
	[MMR_V2_RFLOWRT] = "ringrt",
};
...
static int udma_v2_request(struct dma *dma)
{
...
static int udma_v2_setup_resources(struct udma_dev *ud)
{
	struct udevice *dev = ud->dev;
	int ch_count, ret;
...
static int udma_v2_get_mmrs(struct udevice *dev, struct udma_dev *ud)
{
	int i;
	...
	for (i = 1; i < MMR_V2_LAST; i++) {
		if (i == MMR_V2_BCHANRT && ud->bchan_cnt == 0)
			continue;
		if (i == MMR_V2_CHANRT && ud->chan_cnt == 0)
			continue;

		ud->mmrs[i] = dev_read_addr_name_ptr(dev, mmr_v2_names[i]);
		if (IS_ERR(ud->mmrs[i]))
...
static int udma_v2_rfree(struct dma *dma)
{
	struct udma_dev *ud = dev_get_priv(dma->dev);
	struct udma_chan *uc;
...
static int udma_v2_probe(struct udevice *dev)
{
	...
	ud->dev = dev;
	ret = udma_v2_setup_resources(ud);
	if (ret < 0)
		return ret;
	...
	for (i = 0; i < ud->bchan_cnt; i++) {
		...
		bchan->id = i;
		bchan->reg_rt = ud->mmrs[MMR_V2_BCHANRT] + i * 0x1000;
	}
	...
	for (i = 0; i < ud->tchan_cnt; i++) {
		...
		tchan->id = i;
		tchan->reg_rt = ud->mmrs[MMR_V2_CHANRT] + UDMA_CH_1000(i);
	...
	for (i = 0; i < ud->rchan_cnt; i++) {
		...
		rchan->id = i;
		rchan->reg_rt = ud->mmrs[MMR_V2_CHANRT] + UDMA_CH_1000(i);
	...
	for (i = 0; i < ud->rflow_cnt; i++) {
		...
		rflow->id = i;
		rflow->reg_rt = ud->mmrs[MMR_V2_RFLOWRT] + UDMA_CH_1000(2 * i);
...
static int udma_v2_remove(struct udevice *dev)
{
	struct udma_dev *ud = dev_get_priv(dev);
...
static const struct udevice_id udma_v2_ids[] = {
	{
...
static const struct dma_ops udma_v2_ops = {
	.transfer	= udma_transfer,
	.of_xlate	= udma_of_xlate,
	.request	= udma_v2_request,
	.rfree		= udma_v2_rfree,
	.enable		= udma_enable,
...
U_BOOT_DRIVER(ti_udma_v2) = {
	.name	= "ti-udma-v2",
	.id	= UCLASS_DMA,
	.of_match = udma_v2_ids,
	.ops	= &udma_v2_ops,
	.probe	= udma_v2_probe,
	.remove = udma_v2_remove,
